...
.get(
    ...
    async (c) => {
        ...
        const [currentPeriod] = await fetchFinancialDate(auth.userId,startDate,endDate);
        const [lastPeriod] = await fetchFinancialDate(auth.userId,lastPeriodStart,lastPeriodEnd);
